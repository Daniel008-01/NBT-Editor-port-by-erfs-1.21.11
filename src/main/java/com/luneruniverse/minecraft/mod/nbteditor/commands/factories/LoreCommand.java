...
public class LoreCommand extends ClientCommand {
	...
	public void register(LiteralArgumentBuilder<FabricClientCommandSource> builder, String path) {
		...
		Command<FabricClientCommandSource> list = context -> {
			...
			for (Text line : lore) {
				final int finalI = i;
				Text entry = TextInst.literal("[").formatted(Formatting.GRAY)
					.append(TextInst.literal("-").formatted(Formatting.RED))
					.append(TextInst.literal("]").formatted(Formatting.GRAY))
					...
					.append(TextInst.literal(" ").formatted(Formatting.DARK_PURPLE).formatted(Formatting.ITALIC)
						.append(line)
						.styled(style -> MixinLink.withRunClickEvent(style, () -> MainUtil.client.setScreen(
							new net.minecraft.client.gui.screen.ChatScreen("/factory display lore set " + finalI + " " + FancyTextArgumentType.stringifyFancyText(line, StyleUtil.BASE_LORE_STYLE, true), false)
